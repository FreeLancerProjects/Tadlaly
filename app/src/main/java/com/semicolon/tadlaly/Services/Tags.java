package com.semicolon.tadlaly.Services;

public class Tags {
    public static final String Base_Url="http://tadlaly.tansiq.net/";
    public static final String GeoPlaceUrl="https://maps.googleapis.com/";
    public static final String Image_Url="http://tadlaly.tansiq.net/uploads/images/";
    public static final String update_pass="1";
    public static final String update_name="2";
    public static final String update_email="3";
    public static final String update_phone="4";
    public static final String update_country="5";
    public static final String update_username="6";
    public static final String del="0";
    public static final String upd="1";
    public static final String login_session="1";
    public static final String logout_session="0";
    public static final String sendType_email="0";
    public static final String sendType_whats="1";
    public static final String sendType_normal="2";
    public static final String ad_new="1";
    public static final String ad_old="2";
    public static final String show_phone="1";
    public static final String disApearPhone="2";
    public static final String me_visit="1";
    public static final String visitor="0";
    public static final String share="2";
    public static final String view="1";
    public static final String isSell="1";
    public static final String isdeleted="2";
    public static final String app_user="1";
    public static final String app_visitor="0";
    public static final String reg_from_login="1";
    public static final String reg_from_home="0";
    public static final String display_new="1";
    public static final String display_nearby="2";
    public static final String msg_not_type="message";





}
